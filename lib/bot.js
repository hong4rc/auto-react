 ...
 */
  graph(query) {
    ...
    return BotReact.fetch(url.href);
  }
  ...
  syncWhiteList() {
    return Promise.all([this.updateFriends(), this.updatePageLiked()])
  ...
  parseData(data) {
    ...
    data.forEach((news) => {
      ...
      if (!Object.prototype.hasOwnProperty.call(this.whiteList, from.id)) {
        this.log('❌ %s | %s', id.padStart(ID_FEED_MAX, ' '), from.name);
        return false;
      }
        ...
        .then((res) => {
          ...
          if (err) {
            switch (err.code) {
              ...
              default:
                return this.log('  %s | %s', id.padStart(ID_FEED_MAX, ' '), err.message);
            }
          } else {
            this.ping();
